		...
		<div class="ss-container">
			...
			<div class="grid gap-y-16 gap-x-10 lg:grid-cols-[minmax(0,760px)_320px]">
				...
				<div>
					...
					<!-- Quick Info Card -->
					<div class="mt-8 grid gap-4 sm:grid-cols-3 rounded-2xl border border-slate-200 p-6 text-sm dark:border-zinc-800 bg-white dark:bg-[#262B4E]/40 shadow-sm items-center">
						<div>
							...
							<span class="text-base font-black text-slate-900 dark:text-zinc-50"><?php echo esc_html( $comm_year ? $comm_year : '-' ); ?></span>
						</div>
						<div>
							<span class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-550 mb-0.5"><?php esc_html_e( 'Kota', 'sukusastra' ); ?></span>
							<span class="text-base font-black text-slate-900 dark:text-zinc-50"><?php echo esc_html( $comm_city ? $comm_city : '-' ); ?></span>
						</div>
						<div>
							<span class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-550 mb-0.5"><?php esc_html_e( 'Provinsi', 'sukusastra' ); ?></span>
							<span class="text-base font-black text-slate-900 dark:text-zinc-50"><?php echo esc_html( $comm_prov ? $comm_prov : '-' ); ?></span>
						</div>
					</div>
				...
				<aside class="grid content-start gap-6">
					...
					<div class="ss-card rounded-3xl p-6 grid gap-5 bg-white dark:bg-[#262B4E]/40 shadow-sm border border-slate-200/60 dark:border-zinc-800/80">
						<h3 class="ss-info-title m-0"><?php esc_html_e( 'Hubungi Komunitas', 'sukusastra' ); ?></h3>
						
						<div class="grid gap-4 text-sm">
							<?php if ( $comm_address ) : ?>
								<div>
									<span class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-550 mb-1"><?php esc_html_e( 'Alamat Sekretariat', 'sukusastra' ); ?></span>
									<span class="text-slate-700 dark:text-zinc-300 font-medium leading-relaxed block">
										<?php echo esc_html( $comm_address ); ?>
									</span>
								</div>
							<?php endif; ?>
							...
							<?php if ( $contact ) : ?>
								<div>
									<span class="block text-[10px] font-black uppercase tracking-wider text-slate-400 dark:text-zinc-550 mb-1"><?php esc_html_e( 'Kontak Resmi', 'sukusastra' ); ?></span>
									<?php if ( preg_match( '/^[0-9+]+$/', str_replace( array(' ', '-'), '', $contact ) ) ) : ?>
										...
										?>
										<a href="https://wa.me/<?php echo esc_attr( $clean_phone ); ?>" target="_blank" rel="noopener" class="no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold inline-flex items-center gap-1.5 bg-green-50/60 dark:bg-green-950/20 border border-green-200/50 dark:border-green-800/40 px-3 py-2 rounded-xl text-green-700 dark:text-green-400 transition-colors w-full justify-center">
									...
									<?php elseif ( filter_var( $contact, FILTER_VALIDATE_EMAIL ) ) : ?>
										<a href="mailto:<?php echo esc_attr( $contact ); ?>" class="no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold inline-flex items-center gap-1.5 bg-slate-50 dark:bg-zinc-900 border border-slate-200/60 dark:border-zinc-800 px-3 py-2 rounded-xl transition-colors w-full justify-center">
									...
									<?php else : ?>
										<span class="text-slate-700 dark:text-zinc-300 font-bold leading-normal block"><?php echo esc_html( $contact ); ?></span>
									<?php endif; ?>
								</div>
							<?php endif; ?>
						</div>
					...
					<?php if ( $website || $instagram || $tiktok || $youtube ) : ?>
						<div class="ss-card rounded-3xl p-6 grid gap-5 bg-white dark:bg-[#262B4E]/40 shadow-sm border border-slate-200/60 dark:border-zinc-800/80">
							<h3 class="ss-info-title m-0"><?php esc_html_e( 'Media Sosial', 'sukusastra' ); ?></h3>
							
							<div class="grid gap-3 text-sm">
								<?php if ( $website ) : ?>
									<a href="<?php echo esc_url( $website ); ?>" target="_blank" rel="noopener" class="flex items-center gap-3 no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold border-b border-slate-100 pb-2 dark:border-zinc-800/80">
								...
								<?php if ( $instagram ) : ?>
									...
									?>
									<a href="<?php echo esc_url( $insta_url ); ?>" target="_blank" rel="noopener" class="flex items-center gap-3 no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold border-b border-slate-100 pb-2 dark:border-zinc-800/80">
								...
								<?php if ( $tiktok ) : ?>
									...
									?>
									<a href="<?php echo esc_url( $tiktok_url ); ?>" target="_blank" rel="noopener" class="flex items-center gap-3 no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold border-b border-slate-100 pb-2 dark:border-zinc-800/80">
								...
								<?php if ( $youtube ) : ?>
									<a href="<?php echo esc_url( $youtube ); ?>" target="_blank" rel="noopener" class="flex items-center gap-3 no-underline text-slate-700 dark:text-zinc-300 hover:text-red-700 dark:hover:text-red-400 font-bold border-b border-slate-100 pb-2 dark:border-zinc-800/80">
