<?php
/**
 * Event Gallery Carousel / Single image renderer.
 *
 * @package Poetzen
 */
...
// If we have a gallery, we prepend the featured image to it so that the featured image is the first slide!
if ( has_post_thumbnail( $post_id ) ) {
	$featured_id = get_post_thumbnail_id( $post_id );
	if ( ! in_array( $featured_id, $gallery_array ) ) {
		array_unshift( $gallery_array, $featured_id );
	}
}
...
$carousel_id = 'ss-event-carousel-' . wp_generate_uuid4();
...
<div class="relative w-full overflow-hidden group/carousel">
		...
		<div id="<?php echo esc_attr( $carousel_id ); ?>" class="flex w-full overflow-x-auto scroll-smooth snap-x snap-mandatory no-scrollbar" style="scrollbar-width: none; -ms-overflow-style: none;">
			<?php foreach ( $gallery_array as $index => $img_id ) : ?>
				<div class="w-full shrink-0 snap-start snap-always flex justify-center items-center">
					<?php echo wp_get_attachment_image( (int) $img_id, 'large', false, array( 'class' => 'w-full h-auto rounded-xl object-contain shadow-sm max-h-[500px]' ) ); ?>
				</div>
			<?php endforeach; ?>
		</div>
	...
	<?php else : ?>
		...
		<div class="flex justify-center items-center">
			<?php echo wp_get_attachment_image( (int) $gallery_array[0], 'large', false, array( 'class' => 'w-full h-auto rounded-xl object-contain shadow-sm max-h-[500px]' ) ); ?>
		</div>
	<?php endif; ?>
</div>
